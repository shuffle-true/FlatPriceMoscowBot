...
async def get_link_cian(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("*Произошла непредвиденная ошибка. Поправим в следующих версиях.*", parse_mode="Markdown")
        await state.finish()
        return
...
"Но я исправлюсь, обещаю..)*", parse_mode="Markdown")
        flag = False
        await state.finish()
    ...
    if flag:
        ...
        await message.answer(f"*Decision Tree O(1): {ans[0]} руб.\n"
                             ...
                             f"Standart Deviation: {round(np.var(ans) ** 0.5, 0)}*", parse_mode='Markdown',
