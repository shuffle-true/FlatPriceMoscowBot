...
def df_append(soup, df_):
    ...
    for i in range(len(fio.res_info(soup))):
        flat_dict[fio.res_info(soup)[i].findAll('span', attrs={'class': 'a10a3f92e9--name--x7_lt'})[0].text] = \
        fio.res_info(soup)[i].findAll('span', attrs={'class': 'a10a3f92e9--value--Y34zN'})[0].text

    for i in range(len(fio.house_info(soup))):
        flat_dict[fio.house_info(soup)[i].findAll('div', attrs={'class': 'a10a3f92e9--name--pLPu9'})[0].text] = \
        fio.house_info(soup)[i].findAll('div', attrs={'class': 'a10a3f92e9--value--G2JlN'})[0].text

    for i in range(len(fio.square_floor(soup))):
        flat_dict[fio.square_floor(soup)[i].findAll('div', attrs={'class': 'a10a3f92e9--info-title--JWtIm'})[0].text] = \
        fio.square_floor(soup)[i].findAll('div', attrs={'class': 'a10a3f92e9--info-value--bm3DC'})[0].text.replace(
            '\xa0', ' ')
    ...
    try:
        flat_dict['Холодильник'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--fridge--MD46C'})[0].text
    ...
    try:
        flat_dict['Посудомоечная машина'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--dishwasher--gOo3q'})[0].text
    ...
    try:
        flat_dict['Стиральная машина'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--washing--qc1qA'})[0].text
    ...
    try:
        flat_dict['Мебель в комнатах'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--furniture--Sr1eM'})[0].text
    except IndexError:
    ...
    try:
        flat_dict['Мебель на кухне'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--kitchen--vWMSE'})[0].text
    ...
    try:
        flat_dict['Кондиционер'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--cond--aoNs9'})[0].text
    ...
    try:
        flat_dict['Телевизор'] = soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--tv--pjPm3'})[
            0].text
    ...
    try:
        flat_dict['Интернет'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--internet--y7V4t'})[0].text
    ...
    try:
        flat_dict['Ванна'] = soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--bath--T6AIO'})[
            0].text
    ...
    try:
        flat_dict['Душевая кабина'] = \
        soup.findAll('li', attrs={'class': 'a10a3f92e9--item--CqUH9 a10a3f92e9--shower--zGoTh'})[0].text
    ...
    dictionary = geocoder(
        'Москва, {}'.format(str(flat_dict['street']) + ' ' + str(house_preobras(flat_dict['house'])))).raw
